	...
	public static getRequest(request_info: {
		...
		if (request_info.request_id) {
			...
			if (request_info.status_code) {
				return {
					query: "SELECT request_id, user_id, offer_id, status_id, from_date, to_date, message, qr_code_id FROM request WHERE user_id = ? AND status_id = ? ORDER BY created_on DESC;",
					args: [
